        ...
        with torch.no_grad():
            ...
            for epoch in range(self.ppo_epochs):
                ...
                for t in range(T):
                    ...
                    # get fresh log probs
                    new_log_prob_s = scout_dist.log_probs(buffer.scout_actions[t]).sum(dim=-1)
                    new_log_prob_i = interc_dist.log_probs(buffer.interc_actions[t]).sum(dim=-1)
                    ...
                    epoch_ratio_s += ratio_s.item()
                    epoch_ratio_i += ratio_i.item()
                ...
                # clam log_std to prevent entropy collapse
                with torch.no_grad():
